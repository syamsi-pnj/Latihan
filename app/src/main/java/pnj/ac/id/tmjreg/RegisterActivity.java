...
public class RegisterActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    String nama, email, jamLahir, bod, password, data;
    Button actionSimpan;
        ...
        switch (v.getId()){
            ...
            case R.id.actionSimpan:
                if(edtNama.getText().toString().length() > 0 && edtEmail.getText().toString().length() > 0
                    && edtPassword.getText().toString().length()>0 && edtBod.getText().toString().length()>0
                    && edtJamLahir.getText().toString().length()>0){
                    simpan();
                    Intent intent = new Intent(RegisterActivity.this, LoginActivity.class);
                    startActivity(intent);
                } else {
                    Toast.makeText(this,"Tolong Dilengkapin Datanya!",Toast.LENGTH_SHORT).show();
                }
                break;
    ...
    void simpan(){
        ...
        jamLahir = edtJamLahir.getText().toString();
        bod = edtBod.getText().toString();
        password = edtPassword.getText().toString();

        data = email +" "+ password +" "+ nama +" "+  jamLahir +" "+ bod;
